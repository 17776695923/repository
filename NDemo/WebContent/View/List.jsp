	...
	<div id="main" class="wrapfix">
		<div id="mostlyCon">
			<div class="newslist">
				...
				<div class="bd">
					<ul class="list">
						<c:forEach items="${NewsByTypePage.data }" var="news">
			...
			<div class="paging wrapfix">
				<div class="total">共有${NewsByTypePage.rows }条记录&nbsp;当前${NewsByTypePage.curPage }/${NewsByTypePage.totalPage }页</div>
				<div class="pn">
					<c:choose>
						<c:when test="${NewsByTypePage.curPage <= 1 }">上一页</c:when>
						<c:otherwise><a href="findNewsByType.do?t_id=<%=request.getParameter("t_id") %>&curPage=${NewsByTypePage.curPage-1 }" title="上一页" class="nobar">上一页</a></c:otherwise>
					</c:choose>
					<%
					...
					<%}else{%>
						<a href="findNewsByType.do?t_id=<%=request.getParameter("t_id") %>&curPage=<%=i %>"><%=i %></a>
					<%}} %>
					<c:choose>
						<c:when test="${NewsByTypePage.curPage < NewsByTypePage.totalPage }">
							<a href="findNewsByType.do?t_id=<%=request.getParameter("t_id") %>&curPage=${NewsByTypePage.curPage+1 }" title="下一页" class="nobar">下一页</a>
						</c:when>
